...
// 定义一个用户详情服务数据接口
func (s *server) GetUserInfo(ctx context.Context, req *user.UserInfoRequest) (*user.UserInfoResponse, error) {
	// 修改结构体字面量的字段赋值语法，正确的语法应该使用 . 来指定字段名
	return &user.UserInfoResponse{Id: 1, Name: "xuqing"}, nil
}

// 定义一个用户列表服务数据接口
func (s *server) GetUserList(ctx context.Context, req *user.UserListRequest) (*user.UserListResponse, error) {
